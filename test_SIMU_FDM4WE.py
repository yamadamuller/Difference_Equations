...
def FDM_we(I,V,f,c,L,dt,C,T,new_velo,veloc='constant'): #constant velocity by default
    ...
    for i4 in range(0, Nx + 1):
        ...
        if i4>0:
            ...
            im1 = i4+1
        u[i4] = u_n[i4] + dt2*V(x[i4]) + 0.5*C2*(u_n[i4-1] - 2*u_n[i4] + u_n[ip1]) + 0.5*dt2*f(x[i4], t[0])
    ...
    for n in range(1,Nt):
        if veloc=='constant':
            for i3 in range(1, Nx):
                u[i3] = -u_nm1[i3] + 2*u_n[i3] + C2*(u_n[i3-1] - 2*u_n[i3] + u_n[i3+1]) + dt2*f(x[i], t[n])
        else:
            for i3 in range(1, Nx - 1):
                u[i3] = -u_nm1[i3] + 2 * u_n[i3] + C2 * (0.5*(q[i3]+q[i3+1])*(u_n[i3+1] - u_n[i3]) - \
                0.5*(q[i3] + q[i3-1])*(u_n[i3] - u_n[i3-1]))+ dt2 * f(x[i], t[n])
...
def run(I,V,f,c,L,dt,C,T):
    new_velo = 200  # new velocity in m/s
    u, x, t = FDM_we(I,V,f,c, L, dt, C, T,new_velo,veloc='constant')
