...
def FDM_we(I,V,f,c,L,dt,C,T,veloc=None,new_velo=None): #constant velocity by default
    ...
    for n in range(1,Nt):
        if veloc is None:
            ...
            for i3 in range(1, Nx - 1):
                ...
                0.5*(q[i3] + q[im2])*(u_n[i3] - u_n[im2]))+ dt2 * f(x[i], t[n]) #corrigir equação
...
def run(I,V,f,c,L,dt,C,T):
    ...
    # veloc=True: variable velocity in the medium (None by default)
    #new_velo=float: variable velocity in the medium (None by default)
