...
def sim_yield():
    ...
    for k in range(2, Nt):
        ...
        axi.cla()
        axi.imshow(u_0)
        axi.set_xlabel('Discretização x = 10*[mm]')
        axi.set_ylabel('Discretização z = 10*[mm]')
        axi.set_title("t = {} us".format(t_final*1e6))
        plt.pause(0.001)
...
def sim_yield_CRAS():
    ...
    for k in range(2, Nt):
        ...
        axi.set_xlabel('Nx')
        axi.set_ylabel('Nz')
        axi.set_title("Shot {}".format(t_final*1e6))
        plt.pause(0.001)
...
gate_start = 400
idx_max_r = np.argmax(post_proc.envelope(s_rec[idx_r, gate_start:].T + ymax * idx_r))
t_fl = idx_t[gate_start+idx_max_r]
z_r = t_fl*((c_agua/(Ts / dx)))/2
...
for ir in range(len(s_rec)):
     idx_t = (t*Ts)-1e-6
     plt.plot(idx_t[gate_start:]*1e6, (post_proc.envelope(s_rec[idx_r, gate_start:].T + ymax * idx_r)))
     leg.append('Envelope')
     plt.axvline((t_fl)*1e6, linestyle='solid', color='orange')
     leg.append('t_pico = {} us'.format(t_fl*1e6))
     plt.xlabel('Time (us)')
     plt.ylabel('Amplitude')
     plt.legend(leg)
plt.show()
